...
public class PropertyController {
    ...
    public static class ViewUserProperty extends HttpServlet{
        ...
        protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
            ...
            String sql = "select properties.*, property_types.name_vn as property_type_name_vn, property_types.name_kr as property_type_name_kr, districts.name as district_name, provinces.name as province_name from properties inner join property_types on properties.property_type = property_types.id inner join districts on properties.district_id = districts.id inner join provinces on districts.province_id = provinces.id where user_id = ? order by properties.id desc;";
            MyObject user = (MyObject) req.getSession().getAttribute("login");
            String[] vars = new String[]{user.getId()};
            String[] fields = new String[]{"id", "name_vn", "name_kr", "property_type_name_vn", "property_type_name_kr", "description_vn", "description_kr", "price", "floor_numbers", "at_floor", "district_id", "address", "bedrooms", "bathrooms", "area", "hidden", "for_sale", "sold", "created_at", "district_name", "province_name"};
    ...
    public static class UpdateProperty extends HttpServlet{
        ...
        protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
            ...
            String p_id = req.getParameter("p_id");
            String sql = "update properties set name_vn = ?, name_kr = ?, description_vn = ?, description_kr = ? where id = ?";
            String[] vars = new String[]{name_vn, name_kr, description_vi, description_kr, p_id};
            boolean check = DB.executeUpdate(sql, vars);
            if (check){
                req.getSession().setAttribute("mess", "success|Thay đổi thành công.");
            } else {
